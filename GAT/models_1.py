...
class MWE_DGCN(nn.Module):
    def __init__(
            self, n_input, n_hidden, n_output, n_layers, activation, dropout, residual=False, aggr_mode="sum",
            device="cpu"
    ):
...
class GAT(nn.Module):
    def __init__(
        ...
        self.input_fc = nn.Linear(n_hidden, n_heads * n_hidden)
        ####
        #edge_emb = 0
        ####
        if edge_emb > 0:
